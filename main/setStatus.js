module.exports = async (Discord, client) => {
    ...
    client.on('ready', () => {
        ...
        let statusMessageId = client.config.user.beta.statusMessageId;
        if(process.env.token){
            statusMessageId = client.config.user.main.statusMessageId;
        }

        let statusChannel = client.channels.cache.find(ch => ch.id == client.config.logs.statusChannel)
        statusChannel.messages.fetch(statusMessageId).then((statusMessage) => {
            ...
            const setStatus = (statusMessage) => {
                ...
                let embed = new Discord.MessageEmbed()
                    ...
                    .addFields(
                        {name: 'App Owner', value: '' + client.config.developer.tag + ''.toString(), inline: true},
                        {name: 'ㅤ', value: 'ㅤ\nㅤ', inline: true},
